...
import { setEnquires } from "../redux/store";
import { loadEnquires } from "../services/dashboard";

export default function EnquiresPage() {
  const [search, setsearch] = useState("");

  const enquires = useSelector((state: any) => state.data.enquires);
  const { total, page, data, busy } = enquires;
  ...
  const _changePage = (a1: any) => {
    setEnquires({ total, page: a1, data: [], busy: true });
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Enquires</span> ({total})
        </div>
      </div>
