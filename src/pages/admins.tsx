...
import { setAdmins } from "../redux/store";
import { loadAdmins } from "../services/dashboard";

export default function AdminsPage() {
  const [search, setsearch] = useState("");

  const admins = useSelector((state: any) => state.data.admins);
  const { total, page, data, busy } = admins;
  ...
  const _loadDatas = async (page_: number, search_: string) => {
    await loadAdmins(page_, search_);
  };

  const _changePage = (a1: any) => {
    setAdmins({ total, page: a1, data: [], busy: true });
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Admins</span> ({total})
        </div>
      </div>
