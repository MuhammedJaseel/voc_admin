...
import { setTxns } from "../redux/store";
import { loadTxns } from "../services/dashboard";

export default function TxnsPage() {
  const [search, setsearch] = useState("");

  const txns = useSelector((state: any) => state.data.txns);
  const { total, page, data, busy } = txns;
  ...
  const _loadDatas = async (page_: number, search_: string) => {
    await loadTxns(page_, search_);
  };

  const _changePage = (a1: any) => {
    setTxns({ total, page: a1, data: [], busy: true });
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Transactions</span> (
          {total})
        </div>
      </div>
