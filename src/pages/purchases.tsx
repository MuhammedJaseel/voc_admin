...
import { setPurchases } from "../redux/store";
import { loadPurchases } from "../services/dashboard";

export default function PurchasesPage() {
  const [search, setsearch] = useState("");

  const purchases = useSelector((state: any) => state.data.purchases);
  const { total, page, data, busy } = purchases;
  ...
  const _loadDatas = async (page_: number, search_: string) => {
    await loadPurchases(page_, search_);
  };

  const _changePage = (a1: any) => {
    setPurchases({ total, page: a1, data: [], busy: true });
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Purchases</span> ({total})
        </div>
      </div>
