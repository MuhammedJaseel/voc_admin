...
import { setOrders } from "../redux/store";
import { loadOrders } from "../services/dashboard";

export default function OrdersPage() {
  const [search, setsearch] = useState("");

  const orders = useSelector((state: any) => state.data.orders);
  const { total, page, data, busy } = orders;
  ...
  const _loadDatas = async (page_: number, search_: string) => {
    await loadOrders(page_, search_);
  };

  const _changePage = (a1: any) => {
    setOrders({ total, page: a1, data: [], busy: true });
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Orders</span> ({total})
        </div>
      </div>
