...
import { loadCustomers } from "../services/dashboard";

export default function CustomersPage() {
  const [search, setsearch] = useState("");

  const customers = useSelector((state: any) => state.data.customers);
  const { total, page, data, busy } = customers;
  ...
  const _loadDatas = async (page_: number, search_: string) => {
    await loadCustomers(page_, search_);
  };
    ...
    <div className="p-8">
      <div className="flex justify-between">
        <div className="text-xl">
          <span className="text-[#4F8FE1] font-bold ">Customers</span> ({total})
        </div>
      </div>
